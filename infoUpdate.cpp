...
void IMURetriever::run()
{
	...
	while (true)
	{
		...
		/* Attitude Update */
		{
			...
			drone_info.imu.linear_acc.acc_x = imu.linear_acceleration.y();
			drone_info.imu.linear_acc.acc_x = imu.linear_acceleration.z();
...
void GPSRetriever::run()
{
	...
	while (true)
	{
		...
		
		{
			...
			drone_info.global_position.gps_height = gps.altitude;
...
void BaroRetriever::run()
{
	...
	while (true)
	{
		...
		auto baro = client.getBarometerdata(0.03);
		current_time.start();
		long int time_stamp = current_time.msec() + current_time.second() * 1000 + (long int)current_time.minute() * 1000 * 60;

		{
			QMutexLocker data_locker(&drone_info.data_mutex);
			drone_info.baro.altitude = baro.altitude;
...
void ImgRetriever::run()
{
	...
	while (true)
	{
		...
		vector_to_qimage(image_front_rgb, q_image_front_rgb);
		QImage q_image_front_dep;
		vector_to_qimage(image_front_dep, q_image_front_dep);
		QImage q_image_down_rgb;
		vector_to_qimage(image_down_rgb, q_image_down_rgb);		
